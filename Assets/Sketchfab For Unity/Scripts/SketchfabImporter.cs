...
namespace Sketchfab
{
	class SketchfabImporter
	{
		...
		public void loadFromFile(string filepath)
		{
			...
			_importer.setupForPath(_gltfInput, _importDirectory, _currentSampleName, _addToCurrentScene);
            string licensePath = Path.Combine(_importDirectory, "license.txt");
            if (File.Exists(licensePath))
            {
                try
                {
                    File.Delete(licensePath);
                    Debug.Log("Deleted old license.txt to prevent IO conflict.");
                }
                catch (System.Exception e)
                {
                    Debug.LogWarning("Could not delete existing license.txt: " + e.Message);
                }
            }
            _importer.Load();
		}
